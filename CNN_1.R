...
test_x <- train_x[-train_ind,,, ]
train_y <- train_y[train_ind,]
test_y <- train_y[-train_ind,]


smp_size <- floor(0.5 * nrow(test_x))
valid_ind <- sample(seq_len(nrow(test_x)), size = smp_size)
valid_x <- test_x[valid_ind,,, ]
test_x <- test_x[-valid_ind,,, ]
valid_y <- test_y[valid_ind,]
test_y <- test_y[-valid_ind,]
...
if(!data_augmentation) {  
  model %>% fit( train_x,train_y ,batch_size=10,
                 epochs=40,validation_data = list(valid_x, valid_y),
                 shuffle=TRUE)
} else {  
  #Generating images
